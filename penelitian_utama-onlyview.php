...
<?php
				...
				if($nilaivalid=='Valid'){
					...
					$sql_dosen=mysqli_query($kon,"select avg($kolomqdsekarang) from dosen");
					$avg_dosen=mysqli_fetch_assoc($sql_dosen);
					$avg_dosen=implode($avg_dosen);
					...
					$set_update_jml = mysqli_query($kon,"update butir_pertanyaan_dosen set rerata='$avg_dosen' where kode_pertanyaan='$kolomqdsekarang' ");
...
<?php
		...
		<form action="pilih_jurusan_view_analisis.php" method="post">
		<button type="submit" class="btn btn-success">Analisis Masing-masing Jurusan</button>
		</form>
		...
		<form action="report_penelitian.php" method="post" target="_blank">
		<button type="submit" class="btn btn-warning">Download PDF Hasil Penelitian</button>
		</form>
